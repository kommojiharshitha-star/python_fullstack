...
def add_tasks():
    ...
    if not user_email:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO tasks (user_email, title) VAULES (?, ?)", (user_email, title))
        conn.commit()
        conn.close()
    ...
    def update_tasks(task_id):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE taska SET status = ? WHERE id = ?". (status, tasks_id))
        conn.commit()
        conn.close()
    ...
    def delete_tasks(task_id):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM tasks WHERE user_email=? AND is=?",(user_email,task_id))
        conn.commit()
        conn.close()
        return jsonify({"status": "success", "message": "Task updated successfully!"})
